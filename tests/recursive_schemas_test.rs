...
fn test_self_referencing_schema() {
    ...
    registry
        .register("Comment", Schema::object()
...
fn test_mutually_recursive_schemas() {
    ...
    registry
        .register("B", Schema::object()
            .field("value", Schema::integer())
            .optional("a", Schema::ref_("A")))
        .unwrap();

    let result = registry.validate("A", &json!({
        "name": "First A",
        "b": {
            "value": 42,
            "a": {
...
fn test_circular_ref_through_array() {
    ...
    registry
        .register("Tree", Schema::object()
            .field("value", Schema::integer())
            .optional("children", Schema::array(Schema::ref_("Tree"))))
        .unwrap();

    let result = registry.validate("Tree", &json!({
        "value": 1,
        "children": [
            {
                "value": 2,
...
fn test_circular_ref_through_combinator() {
    ...
    registry
        .register("Value", Schema::one_of(vec![
            Box::new(Schema::integer()) as Box<dyn ValueValidator>,
            Box::new(Schema::object()
                .field("nested", Schema::ref_("Value"))) as Box<dyn ValueValidator>,
        ]))
        .unwrap();

    let result = registry.validate("Value", &json!({
        "nested": {
            "nested": 42
        }
    })).unwrap();
...
fn test_complex_nested_structure() {
    ...
    registry
        .register("Directory", Schema::object()
            .field("type", Schema::string())
            .field("name", Schema::string())
